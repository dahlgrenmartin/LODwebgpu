...
//
// One workgroup produces a TILE x TILE patch of one output channel:
//   dispatch(ceil(W/TILE), ceil(H/TILE), N*Cout)
//
...
fn main(@builtin(workgroup_id) wid : vec3<u32>,
  ...
  loop {
    ...
    loop {
      ...
      if (dims[6] == 0) {
        ...
        let wbase = ((co * Cin + c0 + ci) * 3u) * 3u;
        for (var kh : u32 = 0u; kh < 3u; kh = kh + 1u) {
          let row = tbase + (lid.y + kh) * HALO + lid.x;
          acc = acc + tile[row] * wgt[wbase + kh * 3u]
                    + tile[row + 1u] * wgt[wbase + kh * 3u + 1u]
                    + tile[row + 2u] * wgt[wbase + kh * 3u + 2u];
        }
      } else {
        // transposed: weight is [Cin, Cout, 3, 3], window flipped
        let wbase = (((c0 + ci) * Cout + co) * 3u) * 3u;
        for (var kh : u32 = 0u; kh < 3u; kh = kh + 1u) {
          let row = tbase + (lid.y + kh) * HALO + lid.x;
          let wrow = wbase + (2u - kh) * 3u;
          acc = acc + tile[row] * wgt[wrow + 2u]
                    + tile[row + 1u] * wgt[wrow + 1u]
                    + tile[row + 2u] * wgt[wrow];
        }
      }
      ci = ci + 1u;
    }
  ...
  let ow = ow0 + lid.x;
  if (oh < H && ow < W) {
    if (dims[5] == 1) { acc = acc + bias[co]; }
    dst[((n * Cout + co) * H + oh) * W + ow] = acc;
  }
}
